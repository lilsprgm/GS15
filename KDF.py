...
def pad10star1(data, rate):
    ...
    length = len(data) * 8  # longueur en bits (on travail avec des des tableau bytearray
...
def pi(state):
    """Rôle : Réorganise les éléments de la matrice 5×5 pour permuter leurs positions.."""
...
def chi(state):
    """Rôle : Applique une transformation non linéaire pour mélanger les bits."""
...
def iota(state, round_constant):
    """Rôle : Introduit un élément de « salage » (salt) unique pour chaque tour."""
    state[0, 0] ^= np.unpackbits(np.array([round_constant], dtype=np.uint64).view(np.uint8))
    return state


def sha3(data, output_length, nb_squeeze):
    """Implémente Keccak, utilisé dans SHA-3 avec une sortie de longueur donnée en bits. ON rajoute un paramètre
    nb_squeeze qui coresspond au nombre d'essorage après la phase absorption"""
    ...
    # Phase d'absorption
    for i in range(0, len(padded_data), bitrate // 8): #On prend un pas bit rate // parce que notre tableau est de type bytearray donc avec des octets
        block = padded_data[i:i + bitrate // 8]
        for j in range(len(block)):
            state[j] ^= block[j] #On prend le block de la taille bitrate et on XOR avec l'etat initial (au debut 0 puis ensuite on reprend le résultat suivant)
